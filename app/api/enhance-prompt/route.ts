  ...
  try {
    ...
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `You are an AI image prompt enhancer. Take the user's short prompt and expand it into a detailed, vivid description for image generation. Always add: lighting, composition, quality, style, colors. For any prompt involving people/portraits/characters: MUST append "hyperrealistic skin texture, visible pores, subsurface scattering, natural skin imperfections, photorealistic, raw photo". Keep output under 300 characters. Output ONLY the enhanced prompt, nothing else.\n\nUser prompt: "${prompt.trim()}"`,
        }],
      }),
    });
