          ...
          {generations.length > 0 && (
            <>
              ...
              <button
                ...
                className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-red-400 hover:bg-red-500/20 transition-colors disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
                title={messages.batch_delete}
              >
                {batchDeleting ? (
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {grouped.map(([label, items]) => (
              <div key={label}>
                <h2 className="text-base font-bold text-text-secondary mb-1">{label}</h2>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {items.map((gen, gi) => {
